...
namespace xann {
    template<typename T>
    float simple_l2_distance(const turbo::span<uint8_t> &a, const turbo::span<uint8_t> &b) {
    ...
    template<typename T>
    float simple_l2_norm(const turbo::span<uint8_t> &a) {
        const T *pa = reinterpret_cast<const T *>(a.data());
        const T *last = pa + a.size() / sizeof(T);
        const T *lastgroup = last - 3;
        float diff0, diff1, diff2, diff3;
        float d = 0.0;
        while (pa < lastgroup) {
            diff0 = static_cast<float>(pa[0]);
            diff1 = static_cast<float>(pa[1]);
            diff2 = static_cast<float>(pa[2]);
            diff3 = static_cast<float>(pa[3]);
            d += diff0 * diff0 + diff1 * diff1 + diff2 * diff2 + diff3 * diff3;
            pa += 4;
        }
        while (pa < last) {
            diff0 = static_cast<float>(*pa++);
            d += diff0 * diff0;
        }
        return sqrt(d);
    }
    ...
    float simd_distance_l2(const turbo::span<uint8_t> &a, const turbo::span<uint8_t> &b) {
    ...
    template<typename ARCH>
    float simd_norm_l2_sqrt(const turbo::span<uint8_t> &a) {
        using b_type = xsimd::batch<float, ARCH>;
        std::size_t inc = b_type::size;
        std::size_t size = a.size() / sizeof(float);
        // size for which the vectorization is possible
    ...
    template<typename ARCH>
    float simd_norm_l2(const turbo::span<uint8_t> &a) {
        return sqrt(simd_norm_l2_sqrt<ARCH>(a));
    }
